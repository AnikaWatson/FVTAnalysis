...
```{r, echo=FALSE, fig.cap=fig$cap("Growth of spinach leaves1", "A plot of the leaf lengths of two populations of spinach.")}
...
for (l in 1:20){
  Length[1,l] <- RcticG(a1, k1, g1, (l/2)) + MonoError[l]
}
a2 <- 1
k2 <- 1
g2 <- 1
...
for (l in 1:20){
  RcticG2[1,l] <- RcticG(a2, k2, g2, (l/2)) + MonoError[l]
}

plot(y = Length, x = Time, ylim = c(0, 15), xlim = c(0, 10))
points(y = RcticG2, x = inp, pch = 9)
```
...
matplot(y = t(RcticG1), type = 'p', lty = 1, pch = 16, main = "Males", ylab = "Length (cm)", xlab = "Age (weeks)", las = 1, col = alpha(cols, 0.5))
```

Now that we have data we can fit various models to them. Let's begin by fitting a series of polynomials to the data in a process called polynmial regression. For simplicity, we will focus on one individual, from one population of R-ctic Grayling. Let's take the first male, `RcticG1ind1`.
...
plot(y = RcticG1ind1, x = xvalues, main = NULL, ylab = "Length (cm)", xlab = "Age (weeks)", las = 1)
...
```{r, eval=FALSE}
plot.new(y = RcticG1ind1, x = xvalues, main = "Polynomial Regression", ylab = "Length (cm)", xlab = "Age (weeks)", las = 1)
```
...
matplot(y = t(RcticG1), type = 'p', lty = 1, pch = 10, ylab = "Length (cm)", xlab = "Age (weeks)", las = 1, col = 1, ylim = c(0, 2.5))
